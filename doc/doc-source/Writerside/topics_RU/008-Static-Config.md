...
- одиночные примитивы (строки, числа, bool, char)
    - `simpleapi::Config` хранит числовые значения в формате long double, поэтому объявление в структуре поля float с 
    последующей записью в Config, а затем Config в файл, - запишет в файле мусорный конец значения; обратное чтение
    восстановит изначальный float
